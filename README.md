...
*   **Why**: Allows the application to seamlessly switch between different lead sources (Apollo, LinkedIn, Hunter). The codebase interacts only with the abstract `LeadGenerationStrategy.generate_leads()` method, meaning the application doesn't care *how* a lead is fetched, only that it is returned in a standard format.
...
*   **D - Dependency Inversion Principle**: The FastAPI routes depend on the *abstract* interface/factory logic, not on the low-level implementation details of how an Apollo API call works.
...
*Example output for starting the server:*
```bash
