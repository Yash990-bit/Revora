...
export default function LeadsPage() {
  ...
  const uniqueCampaigns = Array.from(
    new Map(
      leads.map((lead: Lead) => [lead.campaign_id, { id: lead.campaign_id, name: lead.campaign }]),
    ).values(),
  );
  ...
  const campaignStats = Object.values(
    leads.reduce(
      (acc, lead) => {
  ...
  const maxLeads = Math.max(
    ...campaignStats.map((c: { campaign: string; count: number }) => c.count),
    1,
  );
      ...
      <div className="bg-[#111111] border border-white/[0.04] rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select className="bg-transparent text-[#f05a28] text-xs font-semibold focus:outline-none cursor-pointer">
                <option value="az" className="bg-[#111111] text-white">
                  A to Z
                </option>
                <option value="za" className="bg-[#111111] text-white">
                  Z to A
                </option>
                <option value="campaign" className="bg-[#111111] text-white">
                  Campaign
                </option>
              </select>
            </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.04]">
                <th className="text-left py-4 px-6 text-[10px] text-[#f05a28] uppercase tracking-[0.1em] font-bold">
                  LEAD NAME
                </th>
                <th className="text-left py-4 px-6 text-[10px] text-[#f05a28] uppercase tracking-[0.1em] font-bold">
                  COMPANY
                ...
                <th className="text-right py-4 px-6 text-[10px] text-[#f05a28] uppercase tracking-[0.1em] font-bold">
                  ACTIONS
                </th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-8 text-white/50">
                    No leads found in database. Create a campaign and generate leads!
                  </td>
                </tr>
              ) : (
                leads.slice(0, 10).map((lead) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-bold shadow-inner ${lead.avatarBg}`}
                        >
                          {lead.avatar}
                        </div>
                        <div>
                          <p className="text-sm font-bold text-white group-hover:text-[#f05a28] transition-colors cursor-pointer">
                            {lead.name}
                          </p>
                    ...
                    <td className="py-4 px-6 text-sm text-white/70">
                      <span className="bg-white/5 py-1 px-2 rounded-md text-white/80 whitespace-nowrap">
                        {lead.campaign}
                      </span>
                    </td>
                    <td className="py-4 px-6">
        ...
        <div className="p-4 flex items-center justify-between border-t border-white/[0.04] bg-[#0A0A0A]">
          <p className="text-xs text-white/40">
            Showing 1 to {Math.min(10, leads.length)} of {leads.length} leads
          </p>
          <div className="flex items-center gap-1">
            <button
              className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/[0.04] text-white/40 transition-colors cursor-not-allowed opacity-50"
              disabled
            >
        ...
        <div className="bg-[#171717] border border-white/[0.04] rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="h-40 flex items-end justify-center gap-3 px-2 z-10 relative">
            {campaignStats.length > 0 ? (
              campaignStats.map((stat: { campaign: string; count: number }, i) => {
                const height = Math.max((stat.count / maxLeads) * 100, 5); // At least 5% height
                const isBest = stat.count === maxLeads && maxLeads > 0;
                return (
                  <div
                    key={i}
                    className="flex-1 max-w-[80px] w-full flex justify-center group h-full items-end pb-2 relative cursor-pointer"
                  >
                    <div
          ...
          <div className="bg-[#111111] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                ...
                <p className="text-white/40 text-sm mt-1">
                  Design your custom email blast. Use{' '}
                  <code className="text-[#f05a28] bg-white/5 px-1 rounded">{'{{name}}'}</code> and{' '}
                  <code className="text-[#f05a28] bg-white/5 px-1 rounded">{'{{company}}'}</code> as
                  dynamic variables.
                </p>
              </div>
              <button
            ...
            <div className="p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-bold text-white/70 uppercase tracking-wider text-[10px]">
