  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white font-syne text-glow">Integrations</h1>
          <p className="text-sm text-gray-400 mt-1">Connect your neural network to external protocols.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
         {['All Protocols', 'Communication', 'Sales', 'Developers', 'Marketing'].map((cat, i) => (
           <button key={cat} className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
             i === 0 ? "bg-[#5B6EFF] text-white" : "bg-white/[0.02] border border-white/[0.05] text-gray-500 hover:text-white"
           }`}>
             {cat}
           </button>
         ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {APP_CONNECT.map((app) => (
          <div key={app.name} className="group p-6 rounded-[32px] bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] hover:border-[#5B6EFF]/30 transition-all duration-300 relative overflow-hidden">
             ...
             <div className="flex items-center justify-between relative z-10">
               <div className="flex items-center gap-4">
                 <div className="w-14 h-14 rounded-2xl bg-black/40 border border-white/[0.05] flex items-center justify-center p-3.5 relative overflow-hidden group-hover:border-[#5B6EFF]/40 transition-colors">
                    <div className="absolute inset-x-0 bottom-0 h-1" style={{ backgroundColor: app.color }}></div>
                 ...
                 <div>
                   ...
                   <p className="text-[10px] font-black uppercase tracking-widest text-[#5B6EFF] mt-2 opacity-60">{app.category}</p>
                 </div>
               </div>
               <div className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-widest ${
                 app.status === "Active" || app.status === "Connected" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-gray-500/10 text-gray-400 border border-gray-500/20"
               }`}>
                 {app.status}
               </div>
             </div>

             <p className="mt-6 font-medium text-sm text-gray-500 leading-relaxed max-w-[85%]">{app.desc}</p>
        ...
        <div className="p-6 rounded-[32px] border-2 border-dashed border-white/[0.05] bg-transparent flex flex-col items-center justify-center text-center space-y-4 hover:border-[#5B6EFF]/30 hover:bg-white/[0.01] transition-all group cursor-pointer">
           ...
           <div>
              ...
              <p className="text-xs text-gray-500 uppercase tracking-tight mt-1">Direct API cluster connection</p>
           </div>
        </div>
      </div>
    </div>
